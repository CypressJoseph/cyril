...
export namespace Cyril {
    ...
    export class Expectation<Subject> implements Container<Subject> {
        ...
        public toBe(expected: Subject): void { this.expectedValue = new ExactValue(expected); }
        ...
        public glom(...path: string[]) {
            let traverse = (result: any, prop: string) => result == null
                ? undefined
                : result.its(prop) //result[prop];
            let destination = path.reduce(traverse, this);
            return destination; //Box.with(destination);
        }
    ...
    export class Environment {
        ...
        /**
         * Verify all ambient links.
         * 
         * @example
         * ```
         *   cyr.expect(2+2).toBe(4)
         *   await cyr.verify() // ✓ okay
         * 
         *   cyr.expect(2+2).toBe(5)
         *   await cyr.verify() // ✕ throws
         * ```
         */
        public async verify() {
            let verifiers = this.links.map(link => this.handleLink(link))
         ...
         */
        public output(): Box<WrappedFunction> {
        ...
        it(arg0: string, arg1: () => void) {
            throw new Error("Cyril.it -- Method not implemented.");
        }

        describe(featureName: string, theSpec: () => void) {
            this.links.push(
                new Specification(featureName)
            )
        }
        ...
        protected async handleLink<T>(link: Container<T>) {
            if (link instanceof Expectation) {
                ...
                console.warn("Ignoring non-expectation link: " + JSON.stringify(link))
            }
        }
    }
